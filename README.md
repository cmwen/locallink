...
Private Edge declarations name their route adapter explicitly. `adapter: tailscale-serve` is the current supported adapter and is recorded in plans, confirmation tokens, and local ownership state. Unknown adapter names are rejected before command generation. This contract is the provider-neutral seam for adding Tailscale+Caddy without coupling onboarding and reconciliation logic to a new command format.
