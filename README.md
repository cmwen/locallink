...
Private Edge declarations name their route adapter explicitly. `adapter: tailscale-serve` supports the complete confirmed apply and reconciliation lifecycle. `adapter: tailscale-caddy` currently generates a loopback-only Caddyfile, stable per-workspace Caddy listener ports, validation commands, and the Tailscale→Caddy→service topology, but keeps apply blocked until LocalLink owns the Caddy runtime atomically. Caddy availability is resolved within the active workspace first: a Caddy service declared in that workspace's Docker Compose file is reported separately from a host `caddy` command, and Compose runtime checks use that workspace as their working directory. Adapter identity is recorded in plans, confirmation tokens, and local ownership state; unknown adapters are rejected before command generation.
