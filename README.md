...
Private Edge declarations name their route adapter explicitly. `adapter: tailscale-serve` supports the complete confirmed apply and reconciliation lifecycle. `adapter: tailscale-caddy` supports Docker Caddy services that mount `/etc/caddy/Caddyfile` from inside the active workspace. LocalLink validates the generated configuration, persists the original Caddyfile and runtime ownership under `.locallink`, starts a stopped Compose service when needed, writes only its marked managed route block, reloads Caddy, reconciles deselected routes, restores the original file, and stops the service only when LocalLink started it. Apply and reconciliation failures restore both the previous Caddyfile and prior running state. Host-installed Caddy is detection-only and is intentionally never treated as an automated runtime. Adapter identity is recorded in plans, confirmation tokens, and local ownership state; unknown adapters are rejected before command generation.
