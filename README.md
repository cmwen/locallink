...
Private Edge declarations name their route adapter explicitly. `adapter: tailscale-serve` supports the complete confirmed apply and reconciliation lifecycle. `adapter: tailscale-caddy` can now validate and reload a workspace-scoped Docker Caddy service when it mounts `/etc/caddy/Caddyfile` from inside that workspace. LocalLink writes only its marked managed route block, binds the confirmation token to the generated configuration, and restores the previous Caddyfile if route application fails. Caddy availability is resolved within the active workspace first: a Caddy service declared in that workspace's Docker Compose file is reported separately from a host `caddy` command, and Compose runtime checks use that workspace as their working directory. Caddy route reconciliation and host-CLI runtime ownership remain blocked until their safety contracts are implemented. Adapter identity is recorded in plans, confirmation tokens, and local ownership state; unknown adapters are rejected before command generation.
