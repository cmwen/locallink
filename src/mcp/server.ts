...
export async function startMcpServer(context: AppContext): Promise<McpServer> {
  ...
  server.registerTool(
    'plan_extension_onboarding',
    {
      ...
      inputSchema: z.object({
        capability: z.enum(['private-edge', 'identity', 'observability']),
        services: z.array(z.string()).optional(),
      }),
    },
    async ({ capability, services }) => textResponse(JSON.stringify(await context.planExtension(capability, services), null, 2)),
  );
